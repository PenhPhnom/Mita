...
namespace SuperScrollView
{
    ...
    [RequireComponent(typeof(ScrollRectExtra))]
    public class LoopGridView : MonoBehaviour, IBeginDragHandler, IEndDragHandler, IDragHandler
    {
        ...
        ScrollRectExtra mScrollRect = null;
        ...
        System.Action<LoopGridViewItem, BaseUIItem, int, int, int> m_OnBaseUIItemByRowColumn;
        System.Func<LoopGridView, int, int, int, int> m_PrefabsItem;
        ...
        public ScrollRectExtra ScrollRect
        {
            get
            {
                return mScrollRect;
        ...
        */
            ...
            LoopGridViewInitParam initParam = null)
        {
            ...
            mScrollRect = gameObject.GetComponent<ScrollRectExtra>();
            if (mScrollRect == null)
            {
            ...
            if (mScrollRect.horizontalScrollbarVisibility == ScrollRectExtra.ScrollbarVisibility.AutoHideAndExpandViewport && mScrollRect.horizontalScrollbar != null)
            {
                Debug.LogError("ScrollRect.horizontalScrollbarVisibility cannot be set to AutoHideAndExpandViewport");
            }
            if (mScrollRect.verticalScrollbarVisibility == ScrollRectExtra.ScrollbarVisibility.AutoHideAndExpandViewport && mScrollRect.verticalScrollbar != null)
            {
        ...
        public void InitGridView(int itemTotalCount,
           System.Action<LoopGridViewItem, BaseUIItem, int, int, int> onBaseUIItemByRowColumn, System.Func<LoopGridView, int, int> prefansItem = null,
           LoopGridViewSettingParam settingParam = null, LoopGridViewInitParam initParam = null)
        {
            ...
            AdjustContainerAnchorAndPivot();
            InitItemPool();
            m_OnBaseUIItemByRowColumn = onBaseUIItemByRowColumn;
            mNeedCheckContentPosLeftCount = 4;
            mCurSnapData.Clear();
            mItemTotalCount = itemTotalCount;
            UpdateAllGridSetting();
        ...
        */
        ...
        /// <summary>
        /// 根据传入的 index 来获取 Prefab
        /// </summary>
        /// <param name="index"></param>
        /// <returns></returns>
        public LoopGridViewItem NewListViewItem(int index = 0)
        {
            string name = mItemPrefabDataList[index].mItemPrefab.name;
            return NewListViewItem(name);
        }
        ...
        */
        ...
        LoopGridViewItem GetNewItemByRowColumn(int row, int column)
        {
            ...
            LoopGridViewItem newItem = GetOnBaseUIItem(itemIndex, row, column);
        ...
        private LoopGridViewItem GetOnBaseUIItem(int itemIndex, int row, int column)
        {
            LoopGridViewItem newItem;
            if (m_OnBaseUIItemByRowColumn != null)
            {
                int parfabIndex = (m_PrefabsItem != null) ? m_PrefabsItem(this, itemIndex, row, column) : 0;
                newItem = NewListViewItem(parfabIndex);
                if (newItem == null) return null;
                BaseUIItem pBaseItem = newItem.GetComponent<BaseUIItem>();
                if (pBaseItem == null) return null;
                if (!newItem.IsInitHandlerCalled)
                {
                    newItem.IsInitHandlerCalled = true;
                    pBaseItem.OnBlind();
                }
                m_OnBaseUIItemByRowColumn?.Invoke(newItem, pBaseItem, itemIndex, row, column);
                return newItem;
            }
            else
            {
                //LoopGridViewItem
                newItem = mOnGetItemByRowColumn(this, itemIndex, row, column);
                if (newItem == null)
                    return null;
            }
